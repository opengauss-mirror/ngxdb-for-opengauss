...
CREATE FUNCTION check_login(p_token character varying, p_actionid integer) RETURNS json
 ...
 AS $$
    ...
    select count(*)
     ...
     where c.operatorid = v_operatorid and c.systemid=v_systemid
    ...
    if v_c > 0 then
      ...
      return gm.returnjson(0,json_build_object('operatorid',v_operatorid,'systemid',v_systemid,'orgid',v_token->>'orgid'));
		...
		with recursive c as (
	select c3.sysactionid,c3.permissiontype from sysinfo.operpermission c2 left join sysinfo.rolepermission c3 on c2.actionid=c3.roleinfoid where c2.permissiontype=2 and c2.operatorid=v_operatorid and c2.systemid=v_systemid
	union 
	select c3.sysactionid,c3.permissiontype from c left join sysinfo.rolepermission c3 on c.actionid=c3.roleinfoid where c.permissiontype=2 and c3.systemid=v_systemid
	)
    select count(*)
      into v_c
...
begin
  ...
  if p_operatorid is null then 
    v_c:=1;loop exit when (v_c=0);
      v_id :=nextval(sysinfo.operinfo_tokeninterval_seq);
...
begin
  ...
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_operatorid;
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
  update sysinfo.operinfo set isused=1 where operatorid=p_operatorid;
  return gm.returnjson(0); 
end;
$$;
...
--
-- Name: operinfoorg_add(character varying, bigint, character varying, smallint, character varying, character varying, character varying, integer, text, character varying, timestamp without time zone, json); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION operinfoorg_add(p_token character varying, p_operatorid bigint, p_operatorname character varying, p_sex smallint, p_phone character varying, p_accounts character varying, p_memo character varying, p_upcode integer, p_headimgurl text, p_nickname character varying, p_birthday timestamp without time zone, p_sysoperorgjson json) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_id int;v_check json;v_mycode int;v_pass varchar;
begin
  ...
  if p_operatorid is null then 
	...
	v_c:=1;loop exit when (v_c=0);
  	v_mycode:=substring(random()::varchar,2,8);
		select count(*) into v_c from sysinfo.operinfo where mycode=v_mycode;
	end loop;
	v_pass:=substring(md5(random()::varchar),2,8);
	delete from sysinfo.sysoperorg where operatorid=v_id;
  insert into sysinfo.sysoperorg( operatorid,sysorgid) select v_id,sysorgid  from (select distinct sysorgid from json_to_recordset(p_sysoperorgjson::json,true) as t(sysorgid int4) where t.sysorgid in (select sysorgid from sysorg where systemid=(v_check->'info'->>'systemid')::integer and isused=1) ) t;
  insert into sysinfo.operinfo( operatorid,operatorname,sex,phone,accounts,pass,memo,isused,mycode,upcode,headimgurl,nickname,birthday) values(v_id,p_operatorname,p_sex,p_phone,p_accounts,v_pass,p_memo,1,v_mycode,p_upcode,p_headimgurl,p_nickname,p_birthday );
  return gm.returnjson(0,json_build_object('id',v_id,'pass',v_pass,'mycode',v_mycode));
end; 
$$;
...
declare v_c int;v_check json;
begin
  v_check:=gm.check_login(p_token,147);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_operatorid and systemid=(v_check->'info'->>'systemid')::integer and isused=1;
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
  update sysinfo.sysoperorg set isused=0 where operatorid=p_operatorid and sysoperorgid not in (select sysoperorgid from json_to_recordset(p_sysoperorgjson,true) as t(sysoperorgid int8,sysorgid int4) where sysoperorgid is not null);
...
--
-- Name: operinfoorg_edit(character varying, bigint, character varying, smallint, character varying, character varying, character varying, integer, text, character varying, timestamp without time zone, json); Type: FUNCTION; Schema: sysinfo; Owner: gm
--
...
CREATE FUNCTION operinfoorg_edit(p_token character varying, p_operatorid bigint, p_operatorname character varying, p_sex smallint, p_phone character varying, p_accounts character varying, p_memo character varying, p_upcode integer, p_headimgurl text, p_nickname character varying, p_birthday timestamp without time zone, p_sysoperorgjson json) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_check json;v_mycode int;
begin
  ...
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
	delete from sysinfo.sysoperorg where operatorid=v_id;
  insert into sysinfo.sysoperorg( operatorid,sysorgid) select v_id,sysorgid  from (select distinct sysorgid from json_to_recordset(p_sysoperorgjson::json,true) as t(sysorgid int4) where t.sysorgid in (select sysorgid from sysorg where systemid=(v_check->'info'->>'systemid')::integer and isused=1) ) t;
  update sysinfo.operinfo set operatorname=p_operatorname,sex=p_sex,phone=p_phone,accounts=p_accounts,memo=p_memo,mycode=coalesce(mycode,v_mycode),upcode=p_upcode,headimgurl=p_headimgurl,nickname=p_nickname,birthday=p_birthday where operatorid=p_operatorid;
  return gm.returnjson(0);
end;
$$;
...
CREATE FUNCTION operinfoorg_merge(p_token character varying, p_operatorid bigint, p_operatorname character varying, p_sex smallint, p_phone character varying, p_accounts character varying, p_memo character varying, p_upcode integer, p_headimgurl text, p_nickname character varying, p_birthday timestamp without time zone, p_sysoperorgjson json) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_id int;v_check json;v_mycode int;
begin
  v_check:=gm.check_login(p_token,146);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
	v_c:=1;loop exit when (v_c=0);
  	v_mycode:=substring(random()::varchar,2,8);
		select count(*) into v_c from sysinfo.operinfo where mycode=v_mycode;
  ...
  select count(*) into v_c from sysinfo.operinfo where operatorid=v_id and systemid=(v_check->'info'->>'systemid')::integer and isused=1;
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
	delete from sysinfo.sysoperorg where operatorid=v_id;
  insert into sysinfo.sysoperorg( operatorid,sysorgid) select v_id,sysorgid  from (select distinct sysorgid from json_to_recordset(p_sysoperorgjson::json,true) as t(sysorgid int4) where t.sysorgid in (select sysorgid from sysorg where systemid=(v_check->'info'->>'systemid')::integer and isused=1) ) t;
  with "te" as (update sysinfo.operinfo set operatorname=p_operatorname,sex=p_sex,phone=p_phone,accounts=p_accounts,pass=p_pass,memo=p_memo,mycode=coalesce(mycode,v_mycode),upcode=p_upcode,headimgurl=p_headimgurl,nickname=p_nickname,birthday=p_birthday where operatorid=v_id returning *)
  insert into sysinfo.operinfo ( operatorid,operatorname,sex,phone,accounts,pass,tokenkey,tokentime,memo,isused,mycode,upcode,headimgurl,nickname,birthday,tokentype,tokeninterval) select v_id,p_operatorname,p_sex,p_phone,p_accounts,p_memo,1,v_mycode,p_upcode,p_headimgurl,p_nickname,p_birthday where (select count(*) from te) = 0;
  return gm.returnjson(0,json_build_object('id',v_id));
end; 
$$;
...
declare v_c int;v_return json;v_check json;
begin
  v_check:=gm.check_login(p_token,149);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
...
declare v_c int;v_check json;
begin
  v_check:=gm.check_login(p_token,148);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_operatorid and systemid=(v_check->'info'->>'systemid')::integer and isused=0;
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
  update sysinfo.sysoperorg set isused=1 where operatorid=p_operatorid;
  update sysinfo.operinfo set isused=1 where operatorid=p_operatorid;
  return gm.returnjson(0); 
end;
$$;
...
begin
  ...
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
  insert into sysinfo.operpermission( operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,permissionid,systemid) select p_operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,nextval('sysinfo.operpermission_permissionid_seq'),v_check->'inof'->>'systemid' from json_to_recordset(p_operpermissionjson::json,true) as t(permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,permissionid int4,systemid int4);
...
begin
  ...
  select count(*) into v_c from sysinfo.operinfo t1 left join sysinfo.sysoper t2 on t1.operatorid=t2.operatorid where t1.operatorid=p_operatorid and t1.isused=1 and t2.systemid=(v_check->'info'->>'systemid')::integer;
  if v_c=0 then return gm.returnjson(100011); end if;--操作员信息出错！
  delete from sysinfo.operpermission where operatorid=p_operatorid and systemid=(v_check->'info'->>'systemid')::integer and permissionid not in (select permissionid from json_to_recordset(p_operpermissionjson,true) as t(permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,permissionid int4,systemid int4) where permissionid is not null);
  with up as (update sysinfo.operpermission tt set permissiontype=t.permissiontype,ifpermission=t.ifpermission,permissionorder=t.permissionorder,params=t.params,sysactionid=t.sysactionid from json_to_recordset(p_operpermissionjson,true) as t(permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,permissionid int4,systemid int4) where tt.operatorid=p_operatorid and t.sysactionid in (select sysactionid from sysinfo.sysaction where systemid=(v_check->'info'->>'systemid')::integer) and tt.permissionid=t.permissionid and tt.systemid=(v_check->'info'->>'systemid')::integer returning tt.*)
  insert into sysinfo.operpermission( operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,permissionid,systemid) select p_operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,nextval('sysinfo.operpermission_permissionid_seq'),systemid  from json_to_recordset(p_operpermissionjson::json,true) as t(permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,permissionid int4,systemid int4) where sysactionid in (select sysactionid from sysinfo.sysaction where systemid=(v_check->'info'->>'systemid')::integer) and not exists(select 1 from up where t.permissionid=up.permissionid);
  return gm.returnjson(0);
end;
$$;
...
begin
  ...
  if p_orgtypeid is null then 
    v_c:=1;loop exit when (v_c=0);
      v_id :=nextval(sysinfo.orgtype_systemid_seq);
    ...
    if v_c>0 then return gm.returnjson(100018); end if;--部门类型信息出错！
    v_id:=p_orgtypeid;
  end if;
...
orgtypename:部门类型名
description:说明
返回:{"orgtypeid":部门类型编号,"orgtypename":部门类型名,"description":说明}';
...
begin
  ...
  if p_roleinfoid is null then 
    v_c:=1;loop exit when (v_c=0);
      v_id :=nextval(sysinfo.roleinfo_systemid_seq);
  ...
  insert into sysinfo.rolepermission( roleinfoid,permissionid,permissiontype,ifpermission,permissionorder,params,sysactionid,systemid) select v_id,nextval('sysinfo.rolepermission_permissionid_seq'),permissiontype,ifpermission,permissionorder,params,sysactionid,systemid  from json_to_recordset(p_rolepermissionjson::json,true) as t(permissionid int4,permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,systemid int4);
...
roleinfoname:角色名
description:描述
rolepermissionjson:角色权限{"permissionid":角色权限编号,"permissiontype":权限类型,"ifpermission":允许,"permissionorder":权限级别,"params":参数,"sysactionid":权限编号,"systemid":系统id,}
返回：{"errorcode":0,"message":"执行成功！","info":id}';
...
begin
  ...
  select count(*) into v_c from sysinfo.roleinfo where roleinfoid=p_roleinfoid and systemid=(v_check->'info'->>'systemid')::integer and isused=1;
  if v_c=0 then return gm.returnjson(100023); end if;--角色信息出错！
  delete from sysinfo.rolepermission where roleinfoid=p_roleinfoid and systemid=(v_check->'info'->>'systemid')::integer and permissionid not in (select permissionid from json_to_recordset(p_rolepermissionjson,true) as t(permissionid int4,permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,systemid int4) where permissionid is not null);
  with up as (update sysinfo.rolepermission tt set permissiontype=t.permissiontype,ifpermission=t.ifpermission,permissionorder=t.permissionorder,params=t.params,sysactionid=t.sysactionid,systemid=(v_check->'info'->>'systemid')::integer  from json_to_recordset(p_rolepermissionjson,true) as t(permissionid int4,permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,systemid int4) where tt.permissionid=t.permissionid and tt.systemid=(v_check->'info'->>'systemid')::integer returning tt.*)
  insert into sysinfo.rolepermission( roleinfoid,permissionid,permissiontype,ifpermission,permissionorder,params,sysactionid,systemid) select v_id,nextval('sysinfo.rolepermission_permissionid_seq'),permissiontype,ifpermission,permissionorder,params,sysactionid,systemid  from json_to_recordset(p_rolepermissionjson::json,true) as t(permissionid int4,permissiontype int2,ifpermission int2,permissionorder int4,params varchar,sysactionid int4,systemid int4) where systemid=(v_check->'info'->>'systemid')::integer and not exists(select 1 from up where t.permissionid=up.permissionid);
...
description:描述
isused:是否使用
返回:{"roleinfoid":角色编号,"roleinfoname":角色名,"description":描述,"isused":显示}';
...
--
-- Name: sysaction_add(character varying, integer, character varying, integer, character varying, integer, integer, smallint); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION sysaction_add(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_id int;v_check json;
begin
  ...
  insert into sysinfo.sysaction( upid,isused,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(coalesce(p_upid,100),1,p_params,p_systemid,v_id,p_sysactionname,p_actionid,coalesce(p_isdefault,1));
  with recursive tree as
...
ALTER FUNCTION sysinfo.sysaction_add(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) OWNER TO gm;

--
-- Name: FUNCTION sysaction_add(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION sysaction_add(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint)
 IS '新增系统权限
params:参数
sysactionid:系统权限编号
sysactionname:权限名
...
--
-- Name: sysaction_merge(character varying, integer, character varying, integer, character varying, integer, integer, smallint); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION sysaction_merge(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
...
begin
  ...
  if position('.'||p_sysactionid||'.' in '.'||v_idpath||'.')>0 then return gm.returnjson(100033);end if;--上级机构不能循环定义
	if p_isdefault<>0 then p_isdefault:=1;end if;
  with "te" as (update sysinfo.sysaction set upid=coalesce(p_upid,100),params=p_params,systemid=p_systemid,sysactionname=p_sysactionname,actionid=p_actionid,isdefault=p_isdefault  where sysactionid=v_id returning *)
  insert into sysinfo.sysaction ( upid,isused,params,systemid,sysactionid,sysactionname,actionid,isdefault) select coalesce(p_upid,100),1,p_params,(v_check->'info'->>'systemid')::integer,v_id,p_sysactionname,p_actionid,p_isdefault  where (select count(*) from te) = 0;
  with recursive tree as
  ( select sysactionid,upid,1 as idlevel,sysactionid::varchar as idpath,t1.systemid from sysinfo.sysaction t1 where upid=0
  union
  select t1.sysactionid,t1.upid,t2.idlevel+1,t2.idpath||'.'||t1.sysactionid,t1.systemid from sysinfo.sysaction t1 inner join tree t2 on t2.sysactionid=t1.upid and t2.systemid=t1.systemid where isused<>0) 
...
ALTER FUNCTION sysinfo.sysaction_merge(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) OWNER TO gm;

--
-- Name: FUNCTION sysaction_merge(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION sysaction_merge(p_token character varying, p_upid integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_systemid integer, p_isdefault smallint)
 IS '合并系统权限
params:参数
sysactionid:系统权限编号
sysactionname:权限名
...
--
-- Name: sysaction_query(character varying, integer, integer, integer, character varying, integer, character varying, integer, character varying, integer, smallint); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION sysaction_query(p_token character varying, p_rows integer, p_page integer, p_upid integer, p_idpath character varying, p_isused integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_isdefault smallint) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
...
begin
  ...
  if v_check->>'errorcode'<>'0' then return v_check; end if;
  if p_rows is null then
    select array_to_json(array_agg(row_to_json(t))) into v_return from (select  t1.isused,t1.upid,t1.isused,t1.params,t1.sysactionid,t1.sysactionname,t1.actionid,tt.sysactionname upname,t1.isdefault from sysinfo.sysaction t1 left join sysinfo.sysaction tt on t1.upid=tt.sysactionid  where (p_isused is null or t1.isused=p_isused) and (p_upid is null or p_upid=t1.upid) and (p_idpath is null or position(p_idpath in t1.idpath)>0)  and (p_isused is null or p_isused=t1.isused) and (p_params is null or position(p_params in t1.params )>0) and t1.systemid=(v_check->'info'->>'systemid')::integer and (p_sysactionid is null or p_sysactionid=t1.sysactionid) and (p_sysactionname is null or position(p_sysactionname in t1.sysactionname )>0) and (p_actionid is null or p_actionid=t1.actionid) and (p_isdefault is null or t1.isdefault=p_isdefault) order by t1.sysactionid ) t;
    v_c:=coalesce(json_array_length(v_return),0);
  else
    select count(*) into v_c from sysinfo.sysaction t1 where (p_isused is null or t1.isused=p_isused) and (p_upid is null or p_upid=t1.upid) and (p_idpath is null or position(p_idpath in t1.idpath)>0)  and (p_isused is null or p_isused=t1.isused) and (p_params is null or position(p_params in t1.params )>0) and t1.systemid=(v_check->'info'->>'systemid')::integer and (p_sysactionid is null or p_sysactionid=t1.sysactionid) and (p_sysactionname is null or position(p_sysactionname in t1.sysactionname )>0) and (p_actionid is null or p_actionid=t1.actionid) and (p_isdefault is null or isdefault=p_isdefault);
    select array_to_json(array_agg(row_to_json(t))) into v_return from (select  t1.isused,t1.upid,t1.isused,t1.params,t1.sysactionid,t1.sysactionname,t1.actionid,tt.sysactionname upname,t1.isdefault from sysinfo.sysaction t1 left join sysinfo.sysaction tt on t1.upid=tt.sysactionid  where (p_isused is null or t1.isused=p_isused) and (p_upid is null or p_upid=t1.upid) and (p_idpath is null or position(p_idpath in t1.idpath)>0)  and (p_isused is null or p_isused=t1.isused) and (p_params is null or position(p_params in t1.params )>0) and t1.systemid=(v_check->'info'->>'systemid')::integer and (p_sysactionid is null or p_sysactionid=t1.sysactionid) and (p_sysactionname is null or position(p_sysactionname in t1.sysactionname )>0) and (p_actionid is null or p_actionid=t1.actionid) and (p_isdefault is null or t1.isdefault=p_isdefault) order by t1.sysactionid limit greatest(p_rows,1) offset greatest(least((p_page-1)*p_rows,(v_c/p_rows-1+abs(v_c % p_rows))*p_rows),0) ) t;
  end if;
  return gm.returnjson(0,json_build_object('total',v_c,'rows',v_return));
end;
$$;
...
ALTER FUNCTION sysinfo.sysaction_query(p_token character varying, p_rows integer, p_page integer, p_upid integer, p_idpath character varying, p_isused integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_isdefault smallint) OWNER TO gm;

--
-- Name: FUNCTION sysaction_query(p_token character varying, p_rows integer, p_page integer, p_upid integer, p_idpath character varying, p_isused integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_isdefault smallint) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION sysaction_query(p_token character varying, p_rows integer, p_page integer, p_upid integer, p_idpath character varying, p_isused integer, p_params character varying, p_sysactionid integer, p_sysactionname character varying, p_actionid integer, p_isdefault smallint)
 IS '查询系统权限
isused:是否使用
params:参数
sysactionid:系统权限编号
...
actionid:权限编号
isdefault:是否默认
返回:{"params":参数,"sysactionid":系统权限编号,"sysactionname":权限名,"actionid":权限编号}';
...
begin
  ...
  select count(*) into v_c from sysinfo.orgtype where p_orgtype=orgtypeid;
...
begin
  ...
  with "te" as (update sysinfo.sysorg set upid=coalesce(p_upid,100),systemid=(v_check->'info'->>'systemid')::integer,sysorgname=p_sysorgname,description=p_description,orgtype=p_orgtype  where sysorgid=v_id returning *)
...
description:说明
orgtype:部门类型id
返回:{"upid":上级id,"isused":显示,"sysorgid":部门id,"sysorgname":部门名称,"description":说明,"orgtype":部门类型id,orgtypename:部门类型名}';
...
--
-- Name: systeminfo_add(character varying, integer, character varying, integer, smallint, character varying, character varying); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION systeminfo_add(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_id int;v_check json;
begin
  ...
  if p_loginname is null then return gm.returnjson(100012); end if;--子系统信息出错
	select count(*) into v_c from sysinfo.systeminfo where loginname=p_loginname;
	if v_c>0 then return gm.returnjson(100012); end if;--子系统信息出错
  if p_prikey is null or length(p_prikey)<8 or length(p_prikey)>16 then return gm.returnjson(100012); end if;--子系统信息出错
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_adminid and isused=1;
  if v_c=0 then return gm.returnjson(100011);end if;--操作员信息出错！
	if p_algorithm<>1 then p_algorithm:=2;else p_algorithm:=1;end if;
  if p_systemid is null then 
    v_c:=1;loop exit when (v_c=0);
      v_id :=nextval(sysinfo.systeminfo_deltime_seq);
  ...
  insert into sysinfo.systeminfo( systemid,systemname,isused,adminid,algorithm,loginname,prikey,createoperator,createtime) values(v_id,p_systemname,1,p_adminid,p_algorithm,p_loginname,p_prikey,(v_check->'info'->>'operatorid')::integer,now() );
	insert into operpermission(operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,permissionid,systemid) select p_adminid,1,null,null,null,generate_series,nextval('sysinfo.operpermission_permissionid_seq'),p_systemid from generate_series(114,149);
	insert into sysinfo.sysoper(operatorid,systemid) select p_adminid,p_systemid where not exists(select * from sysinfo.sysoper where operatorid=p_adminid and systemid=p_systemid);
  return gm.returnjson(0,json_build_object('id',v_id));
...
ALTER FUNCTION sysinfo.systeminfo_add(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) OWNER TO gm;

--
-- Name: FUNCTION systeminfo_add(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION systeminfo_add(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying)
 IS '新增子系统
systemid:系统编号
systemname:系统名
adminid:子系统管理员id(该id不可删除)
...
CREATE FUNCTION systeminfo_del(p_token character varying, p_systemid integer) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_check json;v_operatorid int;
begin
  v_check:=gm.check_login(p_token,101);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
  select count(*) into v_c from sysinfo.systeminfo where systemid=p_systemid;
  if v_c=0 then return gm.returnjson(100012); end if;--子系统信息出错
	select adminid into v_operatorid from sysinfo.systeminfo where systemid=p_systemid;
  update sysinfo.systeminfo set isused=0,deloperator=(v_check->'info'->>'operatorid')::integer,deltime=now() where systemid=p_systemid;
	delete sysinfo.operpermission where operatorid=v_operatorid and sysactionid>=108 and sysactionid<=113 and systemid=p_systemid;
  return gm.returnjson(0); 
end;
$$;
...
--
-- Name: systeminfo_edit(character varying, integer, character varying, integer, smallint, character varying, character varying); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION systeminfo_edit(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_check json;v_systemid int;v_operatorid int;
begin
  v_check:=gm.check_login(p_token,101);
  if v_check->>'errorcode'<>'0' then return v_check; end if;

  if p_loginname is null then return gm.returnjson(100012); end if;--子系统信息出错
	select count(*) into v_c from sysinfo.systeminfo where loginname=p_loginname and systemid<>p_systemid;
	if v_c>0 then return gm.returnjson(100012); end if;--子系统信息出错
  if p_prikey is null or length(p_prikey)<8 or length(p_prikey)>16 then return gm.returnjson(100012); end if;--子系统信息出错
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_adminid and isused=1;
  if v_c=0 then return gm.returnjson(100011);end if;--操作员信息出错！
  select count(*) into v_c from sysinfo.systeminfo where systemid=p_systemid and isused=1;
  if v_c=0 then return gm.returnjson(100012); end if;--子系统信息出错
	select adminid into v_operatorid from sysinfo.systeminfo where systemid=p_systemid;
	if p_algorithm<>1 then p_algorithm:=2;else p_algorithm:=1;end if;
  update sysinfo.systeminfo set systemname=p_systemname,adminid=p_adminid,algorithm=p_algorithm,loginname=p_loginname,prikey=p_prikey,updateoperator=(v_check->'info'->>'operatorid')::integer,updatetime=now() where systemid=p_systemid;
	if v_operatorid!=p_adminid then
	  delete from sysinfo.operpermission where operatorid=v_operatorid and sysactionid>=114 and sysactionid<=149 and systemid=p_systemid and permissiontype=1;
		delete from sysinfo.operpermission where operatorid=p_admin and sysactionid>=108 and sysactionid<=113 and systemid=p_systemid and permissiontype=1;		
		insert into operpermission(operatorid,permissiontype,ifpermission,perminssionorder,params,sysactionid,permissionid,systemid) select p_adminid,1,null,null,null,generate_series,nextval('sysinfo.operpermission_permissionid_seq'),p_systemid from generate_series(114,149) ;
	  insert into sysinfo.sysoper(operatorid,systemid) select p_adminid,p_systemid where not exists(select * from sysinfo.sysoper where operatorid=p_adminid and systemid=p_systemid);
	end if;
  return gm.returnjson(0);
end;
...
ALTER FUNCTION sysinfo.systeminfo_edit(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) OWNER TO gm;

--
-- Name: FUNCTION systeminfo_edit(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION systeminfo_edit(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying)
 IS '修改子系统
systemid:系统编号
systemname:系统名
adminid:子系统管理员id(该id不可删除)
...
--
-- Name: systeminfo_merge(character varying, integer, character varying, integer, smallint, character varying, character varying); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION systeminfo_merge(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_id int;v_check json;v_operatorid int;
begin
  v_check:=gm.check_login(p_token,101);
  if v_check->>'errorcode'<>'0' then return v_check; end if;
  ...
  if p_loginname is null then return gm.returnjson(100012); end if;--子系统信息出错
  if p_prikey is null or length(p_prikey)<8 or length(p_prikey)>16 then return gm.returnjson(100012); end if;--子系统信息出错
  select count(*) into v_c from sysinfo.operinfo where operatorid=p_adminid;
  if v_c=0 then return gm.returnjson(100011);end if;--操作员信息出错！
	if p_algorithm<>1 then p_algorithm:=2;else p_algorithm:=1;end if;
  if p_systemid is null then 
  	select count(*) into v_c from sysinfo.systeminfo where loginname=p_loginname; 
	...
	select count(*) into v_c from sysinfo.systeminfo where systemid=v_id;
	if v_c=0 then 
	  insert into sysinfo.systeminfo ( systemid,systemname,isused,adminid,algorithm,loginname,prikey,createoperator,createtime) select v_id,p_systemname,1,p_adminid,p_algorithm,p_loginname,p_prikey,(v_check->'info'->>'operatorid')::integer,now();
		insert into operpermission(operatorid,permissiontype,ifpermission,perminssionorder,params,sysactionid,permissionid,systemid) select p_adminid,1,null,null,null,generate_series,nextval('sysinfo.operpermission_permissonid_seq'),p_systemid from generate_series(114,149);
	  insert into sysinfo.sysoper(operatorid,systemid) select p_adminid,p_systemid where not exists(select * from sysinfo.sysoper where operatorid=p_adminid and systemid=p_systemid);
	else 
	  update sysinfo.systeminfo set systemname=p_systemname,adminid=p_adminid,algorithm=p_algorithm,loginname=p_loginname,prikey=p_prikey,updateoperator=(v_check->'info'->>'operatorid')::integer,updatetime=now() where systemid=v_id;
		select adminid into v_operatorid from sysinfo.systeminfo where systemid=v_id;
		if v_operatorid!=p_adminid then 
		  delete from sysinfo.operpermission where operatorid=v_operatorid and sysactionid>=114 and sysactionid<=149 and systemid=p_systemid and permissiontype=1;
			delete from sysinfo.operpermission where operatorid=p_admin and sysactionid>=114 and sysactionid<=149 and systemid=p_systemid and permissiontype=1;		
		  insert into operpermission(operatorid,permissiontype,ifpermission,perminssionorder,params,sysactionid,permissionid,systemid) select p_adminid,1,null,null,null,generate_series,nextval('sysinfo.operpermission_permissionid_seq'),p_systemid from generate_series(114,149);
	    insert into sysinfo.sysoper(operatorid,systemid) select p_adminid,p_systemid where not exists(select * from sysinfo.sysoper where operatorid=p_adminid and systemid=p_systemid);
		end if;
	end if;
...
ALTER FUNCTION sysinfo.systeminfo_merge(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) OWNER TO gm;

--
-- Name: FUNCTION systeminfo_merge(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION systeminfo_merge(p_token character varying, p_systemid integer, p_systemname character varying, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying)
 IS '合并子系统
systemid:系统编号
systemname:系统名
adminid:子系统管理员id(该id不可删除)
...
--
-- Name: systeminfo_query(character varying, integer, integer, integer, character varying, integer, integer, smallint, character varying, character varying); Type: FUNCTION; Schema: sysinfo; Owner: gm
--

CREATE FUNCTION systeminfo_query(p_token character varying, p_rows integer, p_page integer, p_systemid integer, p_systemname character varying, p_isused integer, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_return json;v_check json;
begin
  ...
  if p_rows is null then
    select array_to_json(array_agg(row_to_json(t))) into v_return from (select  t1.systemid,ts.systemname,t1.systemname,t1.isused,t1.adminid,t1.algorithm,t1.loginname,t1.prikey,t1.createoperator,toper.operatorname,t1.createtime::varchar,t2.operatorname from sysinfo.systeminfo t1 left join sysinfo.systeminfo ts on t1.systemid=ts.systemid left join sysinfo.operinfo toper on t1.createoperator=toper.operatorid left join sysinfo.operinfo t2 on t2.operatorid=t1.adminid where  (p_systemid is null or t1.systemid=p_systemid) and (p_systemname is null or position(p_systemname in t1.systemname )>0) and (p_isused is null or p_isused=t1.isused) and (p_adminid is null or p_adminid=t1.adminid) and (p_algorithm is null or p_algorithm=t1.algorithm) and (p_loginname is null or position(p_loginname in t1.loginname )>0) and (p_prikey is null or position(p_prikey in t1.prikey )>0) order by t1.systemid) t;
    v_c:=coalesce(json_array_length(v_return),0);
  else
    select count(*) into v_c from sysinfo.systeminfo t1 where  (p_systemid is null or t1.systemid=p_systemid) and (p_systemname is null or position(p_systemname in t1.systemname )>0) and (p_isused is null or p_isused=t1.isused) and (p_adminid is null or p_adminid=t1.adminid) and (p_algorithm is null or p_algorithm=t1.algorithm) and (p_loginname is null or position(p_loginname in t1.loginname )>0) and (p_prikey is null or position(p_prikey in t1.prikey )>0);
    select array_to_json(array_agg(row_to_json(t))) into v_return from (select  t1.systemid,ts.systemname,t1.systemname,t1.isused,t1.adminid,t1.algorithm,t1.loginname,t1.prikey,t1.createoperator,toper.operatorname,t1.createtime::varchar,t2.operatorname from sysinfo.systeminfo t1 left join sysinfo.systeminfo ts on t1.systemid=ts.systemid left join sysinfo.operinfo toper on t1.createoperator=toper.operatorid left join sysinfo.operinfo t2 on t2.operatorid=t1.adminid where  (p_systemid is null or t1.systemid=p_systemid) and (p_systemname is null or position(p_systemname in t1.systemname )>0) and (p_isused is null or p_isused=t1.isused) and (p_adminid is null or p_adminid=t1.adminid) and (p_algorithm is null or p_algorithm=t1.algorithm) and (p_loginname is null or position(p_loginname in t1.loginname )>0) and (p_prikey is null or position(p_prikey in t1.prikey )>0) order by t1.systemid limit greatest(p_rows,1) offset greatest(least((p_page-1)*p_rows,(v_c/p_rows-1+abs(v_c % p_rows))*p_rows),0) ) t;
  end if;
  return gm.returnjson(0,json_build_object('total',v_c,'rows',v_return));
end;
$$;
...
ALTER FUNCTION sysinfo.systeminfo_query(p_token character varying, p_rows integer, p_page integer, p_systemid integer, p_systemname character varying, p_isused integer, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) OWNER TO gm;

--
-- Name: FUNCTION systeminfo_query(p_token character varying, p_rows integer, p_page integer, p_systemid integer, p_systemname character varying, p_isused integer, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying) ; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON FUNCTION systeminfo_query(p_token character varying, p_rows integer, p_page integer, p_systemid integer, p_systemname character varying, p_isused integer, p_adminid integer, p_algorithm smallint, p_loginname character varying, p_prikey character varying)
 IS '查询子系统
systemid:系统编号
systemname:系统名
isused:是否使用
...
loginname:登录名
prikey:密钥
返回:{"total":总记录数,rows":[{"systemid":系统id,"systemname":系统名称,"systemname":系统名,"adminid":子系统管理员id(该id不可删除),"algorithm":加密函数1aes128,2sm4,"loginname":登录名,"prikey":密钥,"createoperator":操作员编号,"createoperatorname":操作员姓名}]}';
...
CREATE FUNCTION systeminfo_undel(p_token character varying, p_systemid integer) RETURNS json
    LANGUAGE plpgsql NOT SHIPPABLE SECURITY DEFINER
 AS $$
declare v_c int;v_check json;v_adminid int;
begin
  ...
  update sysinfo.systeminfo set isused=1,deloperator=(v_check->'info'->>'operatorid')::integer,deltime=now() where systemid=p_systemid;
	select adminid into v_adminid from sysinfo.systeminfo where systemid=p_systemid;
	insert into operpermission(operatorid,permissiontype,ifpermission,permissionorder,params,sysactionid,permissionid,systemid) select v_adminid,1,null,null,null,generate_series,nextval('sysinfo.operpermission_permissionid_seq'),p_systemid from generate_series(114,149);
	insert into sysinfo.sysoper(operatorid,systemid) select v_adminid,p_systemid where not exists(select * from sysinfo.sysoper where operatorid=v_adminid and systemid=p_systemid);
  return gm.returnjson(0); 
end;
$$;
...
CREATE TABLE appparams (
    ...
    isused smallint,
    tokentime timestamp without time zone,
    typeid integer
)
WITH (orientation=row, compression=no);
...
--
-- Name: COLUMN operpermission.systemid; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON COLUMN operpermission.systemid IS '系统id';
...
--
-- Name: COLUMN rolepermission.systemid; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON COLUMN rolepermission.systemid IS '系统id';
...
--
-- Name: COLUMN systeminfo.adminid; Type: COMMENT; Schema: sysinfo; Owner: gm
--

COMMENT ON COLUMN systeminfo.adminid IS '子系统管理员id(该id不可删除)';
...
--
-- Name: operaccounts_pkey; Type: CONSTRAINT; Schema: sysinfo; Owner: gm; Tablespace: 
--

ALTER TABLE operaccounts
    ADD CONSTRAINT operaccounts_pkey PRIMARY KEY  (operatorid, typeid);
...
--
-- Name: gethelp(); Type: ACL; Schema: gm; Owner: gm
--

REVOKE ALL ON FUNCTION gethelp() FROM PUBLIC;
REVOKE ALL ON FUNCTION gethelp() FROM gm;
GRANT EXECUTE ON FUNCTION gethelp() TO gm;
GRANT EXECUTE ON FUNCTION gethelp() TO PUBLIC;
GRANT EXECUTE ON FUNCTION gethelp() TO conn;
...
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,143,'查询员工权限',null,null,'sysinfo/operinfopermission_query',null);
...
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,149,'查询员工部门',null,null,'sysinfo/operinfoorg_query',null);
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,126,'新增员工',null,null,'sysinfo/operinfo_add',null);
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,127,'修改员工',null,null,'sysinfo/operinfo_edit',null);
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,128,'合并员工',null,null,'sysinfo/operinfo_merge',null);
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,129,'删除员工',null,null,'sysinfo/operinfo_del',null);
...
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,130,'恢复员工',null,null,'sysinfo/operinfo_undel',null);
insert into sysinfo.actions(isused,actionid,actionname,description,params,actionurl,code) values(1,131,'查询员工',null,null,'sysinfo/operinfo_query',null);
...
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.141',2,0,100,null,100,141,'删除员工权限',141,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.132',2,0,100,null,100,132,'新增角色',132,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.133',2,0,100,null,100,133,'修改角色',133,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.115',2,0,100,null,100,115,'修改部门',115,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.116',2,0,100,null,100,116,'合并部门',116,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.117',2,0,100,null,100,117,'删除部门',117,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.118',2,0,100,null,100,118,'恢复部门',118,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.119',2,0,100,null,100,119,'查询部门',119,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.134',2,0,100,null,100,134,'合并角色',134,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.143',2,0,100,null,100,143,'查询员工权限',143,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.140',2,0,100,null,100,140,'合并员工权限',140,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.113',2,0,100,null,100,113,'查询系统权限',113,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.135',2,0,100,null,100,135,'删除角色',135,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.137',2,0,100,null,100,137,'查询角色',137,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.111',2,0,100,null,100,111,'删除系统权限',111,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100',1,53,0,null,100,100,null,null,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.124',2,0,100,null,100,124,'恢复部门类型',124,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.125',2,0,100,null,100,125,'查询部门类型',125,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.106',2,0,100,null,100,106,'查询子系统',106,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.139',2,0,100,null,100,139,'修改员工权限',139,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.130',2,0,100,null,100,130,'恢复员工',130,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.131',2,0,100,null,100,131,'查询员工',131,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.103',2,0,100,null,100,103,'合并子系统',103,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.102',2,0,100,null,100,102,'修改子系统',102,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.126',2,0,100,null,100,126,'新增员工',126,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.127',2,0,100,null,100,127,'修改员工',127,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.128',2,0,100,null,100,128,'合并员工',128,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.120',2,0,100,null,100,120,'新增部门类型',120,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.121',2,0,100,null,100,121,'修改部门类型',121,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.101',2,0,100,null,100,101,'新增子系统',null,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.142',2,0,100,null,100,142,'恢复员工权限',142,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.108',2,0,100,null,100,108,'新增系统权限',108,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.109',2,0,100,null,100,109,'修改系统权限',109,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.110',2,0,100,null,100,110,'合并系统权限',110,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.136',2,0,100,null,100,136,'恢复角色',136,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.105',2,0,100,null,100,105,'恢复子系统',105,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.104',2,0,100,null,100,104,'删除子系统',104,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.112',2,0,100,null,100,112,'恢复系统权限',112,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.148',2,0,100,null,100,148,'恢复员工部门',148,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.129',2,0,100,null,100,129,'删除员工',129,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.149',2,0,100,null,100,149,'查询员工部门',149,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.138',2,0,100,null,100,138,'新增员工权限',138,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.122',2,0,100,null,100,122,'合并部门类型',122,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.123',2,0,100,null,100,123,'删除部门类型',123,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.145',2,0,100,null,100,145,'修改员工部门',145,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.146',2,0,100,null,100,146,'修改员工部门',146,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.147',2,0,100,null,100,147,'删除员工部门',147,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.144',2,0,100,null,100,144,'新增员工部门',144,null);
insert into sysinfo.sysaction(isused,idpath,idlevel,idcount,upid,params,systemid,sysactionid,sysactionname,actionid,isdefault) values(1,'100.114',2,0,100,null,100,114,'新增部门',114,null);
...
insert into sysinfo.errorcode(message,errorcode,primekey,isused,schema) values('上级机构不能循环定义！',100017,null,1,null);
...
insert into sysinfo.errorcode(message,errorcode,primekey,isused,schema) values('操作员部门信息出错！',100019,'sysoperorgid',1,'sysinfo.sysoperorg');
...
insert into sysinfo.errorcode(message,errorcode,primekey,isused,schema) values('操作员所属系统信息出错！',100021,'sysoperid',1,'sysinfo.sysoper');
